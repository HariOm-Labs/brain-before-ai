...
```mermaid
flowchart LR
    IN[Information arriving<br/><i>F = m × a</i>]

    subgraph KNOBS[" Modulator state determines fate "]
        DA[🎯 Dopamine<br/><i>this matters?</i>]
        ACH[🔍 Acetylcholine<br/><i>focused?</i>]
        NE[⚡ Norepinephrine<br/><i>alert?</i>]
        SE[🧘 Serotonin<br/><i>patient?</i>]
    end

    OUT_GOOD[✅ Encoded as memory<br/><i>knobs turned up</i>]
